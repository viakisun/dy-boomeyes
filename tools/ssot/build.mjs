...
mkdirSync(GEN_CODE, { recursive: true });
mkdirSync(GEN_DOC, { recursive: true });
...
const L = {
  ...
  kind: { core: '기본', proposed: '제안', option: '옵션' },
  disc: { open: '미결', decided: '확정', dropped: '철회' },
  track: { A: 'A 시급', B: 'B 정책', C: 'C 범위·계약' },
};
const HEAD = (title, src) =>
...
if (existsSync(specDir))
  for (const feat of readdirSync(specDir).sort(cmp)) {
    const p = join(specDir, feat, 'spec.md');
...
const frByScreen = new Map();
for (const f of d.requirements.fr)
  for (const s of f.screens) (frByScreen.get(s) ?? frByScreen.set(s, []).get(s)).push(f.id);
const ifByScreen = new Map();
for (const i of d.interfaces.if)
  for (const s of i.screens) (ifByScreen.get(s) ?? ifByScreen.set(s, []).get(s)).push(i.id);
const discByScreen = new Map();
for (const x of d.decisions.disc)
  for (const s of x.scope) (discByScreen.get(s) ?? discByScreen.set(s, []).get(s)).push(x.id);
  ...
  derived: {
    screens: Object.fromEntries(
      screens.map((s) => [
        s.id,
        {
          fr: (frByScreen.get(s.id) ?? []).sort(cmp),
          if_all: [...new Set([...(s.trace.if ?? []), ...(ifByScreen.get(s.id) ?? [])])].sort(cmp),
          disc_all: [...new Set([...(s.trace.disc ?? []), ...(discByScreen.get(s.id) ?? [])])].sort(cmp),
          specs: specOf(s.id),
        },
      ]),
    ),
...
const constBlock = (name, list, type) =>
...
const ts = [
  ...
  constBlock(
    'FR',
    byId(d.requirements.fr).map((x) => x.id),
    'FrId',
  ),
  constBlock(
    'ENT',
    byId(d.entities.ent).map((x) => x.id),
    'EntId',
  ),
  constBlock(
    'DISC',
    byId(d.decisions.disc).map((x) => x.id),
    'DiscId',
  ),
  constBlock(
    'IF',
    byId(d.interfaces.if).map((x) => x.id),
    'IfId',
  ),
  constBlock(
    'API',
    byId(d.interfaces.api).map((x) => x.id),
    'ApiId',
  ),
...
md +=
  tbl(
    ...
    screens.map((s) => [
      ...
      s.phase,
      s.wave,
      `${s.states.length} (${s.states.map((x) => x.id).join(' ')})`,
      (frByScreen.get(s.id) ?? []).join(' '),
      specOf(s.id).join(' ') || '—',
    ]),
  ) + '\n';
...
for (const [name, m] of Object.entries(d.entities.machines))
  md +=
    ...
    tbl(
      ['from', 'to', '계기'],
      m.transitions.map((t) => [t.from, t.to, t.by]),
    ) +
    '\n\n';
md +=
  '## 규칙\n\n' +
...
md +=
  ...
  tbl(
    ['저장소', '노드', '대상', '용도', '단계'],
    d.entities.stores.map((s) => [s.store, s.node, s.entities, s.purpose, s.phase]),
  ) +
  '\n\n' +
  tbl(
    ['축적 항목', '원천', 'ENT', '계층'],
    d.entities.accumulation.map((a) => [a.item, a.source, a.entities.join(' '), a.tier]),
  ) +
  '\n';
...
md +=
  tbl(
    ...
    [
      ['화면', screens.length],
      ['FR', frs.length],
      ['FR 0건 화면(고아)', orphans.length ? orphans.map((s) => s.id).join(' ') : '0'],
      ['화면 0건 FR', frNoScreen.length ? frNoScreen.map((f) => f.id).join(' ') : '0'],
      [
        `현재 웨이브(${d.meta.current_wave}) 이하 spec 없는 화면`,
        noSpec.length ? noSpec.map((s) => s.id).join(' ') : '0',
      ],
      ['참조 수', check.nrefs],
      ['검사 오류', check.errors.length],
    ],
  ) + '\n\n';
md +=
  '## FR → 화면 · IF · ACC · spec\n\n' +
  tbl(
    ...
    frs.map((f) => [
      ...
      f.if.join(' '),
      f.acc.join(' ') || '—',
      [...new Set(f.screens.flatMap(specOf))].join(' ') || '—',
    ]),
  ) +
  '\n\n';
md +=
  ...
  tbl(
    ...
    screens.map((s) => [
      ...
      s.trace.out ?? '—',
      s.trace.rfp.join(' ') || '—',
      json.derived.screens[s.id].disc_all.join(' ') || '—',
      json.derived.screens[s.id].if_all.join(' ') || '—',
    ]),
  ) +
  '\n\n';
md +=
  ...
  tbl(
    ...
    d.contract.sections.map((sec) => [
      sec.id,
      sec.name,
      sec.phase,
      sec.items.length,
      sec.items.filter((i) => i.screens.length).length,
      sec.items.filter((i) => !i.screens.length).length,
    ]),
  ) +
  '\n';
...
md +=
  tbl(
    ...
    [
      ['미결', open.length],
      ['확정', discs.filter((x) => x.status === 'decided').length],
      ['철회', discs.filter((x) => x.status === 'dropped').length],
      ...['A', 'B', 'C'].map((t) => [`미결 · ${L.track[t]}`, open.filter((x) => x.track === t).length]),
    ],
  ) + '\n\n';
md +=
  tbl(
...
md +=
  tbl(
    ...
    (d.scenarios.demo ?? []).map((s) => [
      ...
      s.message,
      (s.acc ?? []).join(' '),
      Object.entries(s.fixture ?? {})
        .map(([k, v]) => `${k}=${v}`)
        .join(' '),
    ]),
  ) + '\n\n';
const trackScreens = {};
for (const s of d.scenarios.demo ?? []) for (const scr of s.screens) (trackScreens[s.track] ??= new Set()).add(scr);
md +=
  tbl(
    ['트랙', '장면', '화면'],
    Object.keys(trackScreens)
      .sort(cmp)
      .map((t) => [
        t,
        (d.scenarios.demo ?? [])
