...
const PARAMS = { '[case]': 'C-105', '[device]': 'CPB-003', '[site]': 'SITE-001', '[camera]': 'CAM-3-2', '[event]': 'EV-001' };
...
async function waitHttp(url, ms = 30_000) { const t0 = Date.now(); while (Date.now() - t0 < ms) { try { const r = await fetch(url); if (r.ok) return; } catch {} await new Promise((r) => setTimeout(r, 300)); } throw new Error(`서버 응답 없음 ${url}`); }
const servers = [];
if (!args.includes('--no-serve')) for (const app of ['web', 'pwa']) {
  if (!existsSync(join(ROOT, 'apps', app, 'build'))) throw new Error(`apps/${app}/build 없음 — 먼저 pnpm build`);
  servers.push(spawn('pnpm', ['--filter', `@boomeyes/${app}`, 'preview', '--port', String(PORTS[app]), '--strictPort'], { cwd: ROOT, stdio: 'ignore' }));
  await waitHttp(BASE[app]);
}
const browser = await chromium.launch();
const ctx = await browser.newContext({ deviceScaleFactor: 2, locale: 'ko-KR', reducedMotion: 'reduce' });
await ctx.addInitScript((iso) => { const fixed = new Date(iso).getTime(); const D = Date; const P = new Proxy(D, { construct(t, a) { return a.length ? new D(...a) : new D(fixed); }, get(t, k) { return k === 'now' ? () => fixed : Reflect.get(t, k); } }); globalThis.Date = P; Math.random = () => 0.42; }, ssot.meta.fixed_clock);
let n = 0, fail = 0;
for (const s of screens) {
  const app = surfaces[s.surface]; const phone = app === 'pwa';
  for (const st of s.states) {
    ...
    try {
      ...
      const dialog = await page.$('[data-capture-dialog]'); const frame = await page.$('[data-capture-frame]');
      const target = dialog ?? (phone ? frame : null);
      if (target) await target.screenshot({ path: join(OUT, `${name}.png`) }); else await page.screenshot({ path: join(OUT, `${name}.png`), fullPage: !phone });
      n++; console.log('ok', name);
    } catch (e) { fail++; console.log('FAIL', name, String(e.message).split('\n')[0]); }
    await page.close();
  }
}
await browser.close(); for (const p of servers) p.kill();
