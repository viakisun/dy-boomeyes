...
SUPPLEMENTS = [
    ...
    # 호기 카드는 제 상자 안에서 스크롤한다 — 문서 높이가 늘지 않으므로 캡처에는 첫 화면만큼만 담긴다.
    # 제목은 담긴 것만 말한다(아래로 이어지는 지표·이상 이력·서류는 해설이 설명한다).
    ("web", "overview", "unit", "호기 화면의 카드 — 오늘 운전자·AI 경고·계약", (78, 195, 444, 640), True),
