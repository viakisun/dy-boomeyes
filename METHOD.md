# BoomEyes AI 개발 방법론 v1.2 (2026-09-06 · W2 회고 반영)
...
- **머지 규칙**(v1.1): 브랜치는 `pnpm verify` 녹색과 reviewer 판정 "머지 가능" 인용 둘 다 있어야 merge. 판정 대기 중엔 다음 브랜치에서 작업한다(W0 1회 위반 → CLAUDE.md SOP). 판정 요지(판정 · 심각도별 건수)는 `--no-ff` merge 커밋 본문에 인용해 리포에 남긴다 — W0 회고의 리뷰 지표가 세션 로그에만 있어 재검증되지 않았다.
- **명령 체인**(v1.1): 게이트 → 커밋은 `&&`로만 잇는다. `;`로 이으면 빨간 게이트를 지나 커밋된다(W0 2회).
- **화면 안 쿼리 전환**(v1.2): 칩·탭·기간 같은 `?x=` 전환은 `new URL(location.href)`로 다른 파라미터(`?state=` `?capture=` `?scene=`)를 유지한다 — 버리면 mock db 키가 live로 바뀌어 픽스처가 사라진다(W2 B8).
- **e2e 단언**(v1.2): 토스트는 쌓이므로 `getByRole('status').filter({ hasText })` · 폼 컨트롤 라벨은 컨트롤 바깥 `<label for>`(감싸면 접근 가능한 이름에 option 텍스트가 섞인다) · 같은 화면 코드로 axe 항목을 둘 넣지 않는다(제목 중복) · 시각 문자열은 `fmtDateTime`(Asia/Seoul), ISO `slice`는 UTC(W2 B5·B7·B9).
- **자리 화면 게이트**(v1.2): `pnpm capture --dark --strict`가 캐치올 자리 화면을 FAIL로 센다 — 웨이브 Exit "자리 0"은 이 게이트로만 증명한다(`ssot:check`는 route 필드만 본다).
- **리뷰 에이전트 운영**(v1.2): 게이트 명령은 파일 리다이렉트 + exit code로만 판정하게 지시하고, 600초 무응답이면 포트 리스너·잔여 프로세스를 확인한 뒤 새 에이전트로 재리뷰한다(W2 B10).
...
| 캡처 | 라우트 × 상태 전수(웨이브 이하) · 셸 포함 · TZ 고정 · 지도 idle 실패 집계 · `--strict` 자리 화면 = FAIL(W2) · 시각 회귀 기준선은 ADR-008 | PR 차단 |
...
- W2 실측(9/5~9/6): 브랜치 B0~B11·B15 = PR 13(#2~#14) 전부 reviewer 판정 인용 + CI 녹색 뒤 merge · 리뷰 "머지 불가" 1회(B3, 후속 커밋으로 해소) · 재리뷰 1회(B10, 에이전트 stall) · 화면 41 실화면 · e2e 78 → 141 · axe 41 · 캡처 41 → 97 · 픽스처 17 → 44 · 상세 `docs/retro/W2.md`.
- W0 실측(9/5): ff-merge 브랜치 16(`git reflog show main`) · 첫 판정 "머지 가능" 2건(`chore/w1-polish` · `fix/w1-wall-channels`) · 리뷰가 잡은 결함 17(세션 기록: 미구현 AC 6 · 게이트 우회 3 · 데이터 유실/비반응 3 · 문서 드리프트 3 · AX-1 우회 2) · 사고 SOP 11건. 상세 `docs/retro/W0.md`.
...
- v1.2(2026-09-06): W2 회고(`docs/retro/W2.md`) 반영 — §5 쿼리 전환·e2e 단언·자리 화면 게이트·리뷰 에이전트 운영 · §6 캡처 `--strict` · §7 W2 실측.
