...
코드는 읽기 좋게 줄을 나눴으므로 시안과 **줄 단위로는** 맞지 않는다 — 대조는 화면과 의미로 한다.

## 옮기면서 바꾼 것

1. 시안이 `/* legacy (unused) */`로 묶어 둔 블록과 `*_old` 함수들, 그리고 어디서도 쓰이지 않는
   CSS 클래스 23개를 뺐다.
2. 생성 HTML이 `onclick="goTab('ops')"` 꼴을 쓰므로 `main.ts` 끝에서 그 함수들을 `window`에 얹는다.
   **인라인 핸들러가 부르는 이름은 반드시 거기 있어야 한다** — 빠지면 클릭이 조용히 죽는다.
3. 타입 추론을 푸는 표기 몇 곳(`SITES: any[]`, `$(sel): any` 등). `tsconfig`는 느슨하다 —
   시안 코드에 타입을 채우면 코드가 몇 배로 는다.
4. 한 줄에 밀어 넣은 템플릿 리터럴을 나누고 되풀이되는 판정에 이름을 붙였다. HTML을 바꾸지 않으려고
   줄바꿈 대신 `+` 연결을 쓴다 — 템플릿 안에서 줄을 바꾸면 태그 사이에 공백이 끼어든다.
5. 배정 확정을 동작시켰다(`new` → `assign`). 시안에는 버튼만 있고 핸들러가 없었다.
