...
|---|---|---|---|
| 진입 B0-01 / A4-01 | 로그인 카드(로고 · "소유주 로그인" · 데모 시작 버튼) — 랜딩·미리보기 없음(사용자 결정 2026-09-12) | 히어로 카피 · 미리보기 · "예시" 라벨 · 각주 | 로고 두 톤 · 단일 행동 |
...
- 진입은 C3에서 실제 컴포넌트 미리보기로 만들었다가 사용자 결정(2026-09-12 "랜딩이 아니라 로그인 화면")으로 로고 · 제목 · 데모 시작 버튼만 있는 로그인 카드로 교체했다(`feat/owner-entry-login`). 영상 화면의 lucide 아이콘은 ui 재수출(`IconPlay` 등)로 쓴다(video 패키지는 lucide 의존이 없다).
