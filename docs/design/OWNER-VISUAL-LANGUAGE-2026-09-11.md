...
| 호기 화면은 어디서 들어와도 같은 화면 하나 | 현황의 호기 단계가 그 화면이다. 지도 자리를 카메라 6분할이 대신한다. **2026-09-14에 실제로 하나로 만들었다** — 옛 독립 상세 화면 둘(웹·PWA)을 폐기하고 목록·알림·운전자·서류·영상의 길을 모두 호기 단계로 돌렸다 |
...
### 호기 화면을 하나로 합치며(2026-09-14) 드러난 것

사용자 지적에서 출발했다 — 「시안은 전체 지도 → 현장 지도 → 호기 상세 정보인데 반영이 안 됐고, 예전 자료의 호기 상세만 있다」.

- **적어 놓고 하지 않았다.** §11 표 442행에 「호기 화면은 하나」라고 적고도 옛 독립 상세를 그대로 뒀다. 보유 장비 표·알림·운전자가 전부 옛 화면으로 갔고, 호기 단계에는 거기로 나가는 버튼까지 있었다. **문서에 적는 것과 코드에서 지우는 것은 다른 일이다.**
- **검토안이 드릴다운을 숨기고 있었다.** `build-review.py`의 `context()`가 `level`이 `nation`인 샷만 본문 장면으로 써서, 「02 운영 현황」이 전국 한 장이었고 현장·호기는 뒤쪽 확대 페이지에 묻혔다. 구현은 3단계인데 **문서만 보면 없는 것처럼 읽혔다** — 사용자가 본 것이 그것이다.
- **없앨 화면에만 있던 것이 요구였다.** 수신이 끊긴 호기의 지표는 호기 단계에서 전부 「미연동」인데(옛 값을 지금 값처럼 보이지 않게 · FR-034), 마지막으로 받은 전압을 보이던 곳은 옛 상세뿐이었다. 지웠으면 FR-034의 표현과 오프라인 시험이 함께 사라졌다. **지우기 전에 무엇을 지고 있었는지 먼저 센다.**
- **시험이 옛 화면의 마크업에 기대고 있었다.** `page.locator('video')` 0건으로 「플레이어가 풀렸다」를 보던 것이 카메라 벽 6타일에 걸렸고, `not.toContainText('0 V')`가 「380 V」의 끝자리에 걸렸으며, `innerText`를 떠 놓고 `toHaveText`로 견주던 비교가 `<dl>` 마크업에서 어긋났다. 셋 다 옛 화면에서는 우연히 맞던 것이다.
- **캡처가 보여 줄 수 있는 것이 줄었다.** 옛 상세는 페이지 전체가 스크롤이라 「전체 높이 캡처」가 길었지만, 호기 카드는 제 상자 안에서 스크롤해 문서 높이가 늘지 않는다 — 카드 아래쪽은 어떤 캡처에도 담기지 않는다. 확대 페이지 제목이 담기지 않은 것을 적고 있어 바로잡았다.

남긴 것: 영상 주소 `/b1/fleet/[device]/video`는 부모(`/b1/fleet/[device]`)가 사라졌지만 그대로 둔다 — 화면마다 앵커된 정규식으로 매칭하므로 동작하고, 주소를 옮기면 SSOT·e2e·캡처가 또 흔들린다. 미니맵과 장비 스틸 포스터는 사용자 결정으로 버렸다. 지난 커밋의 옛 화면 ID 참조는 그대로 남으므로 과거 범위로 `check --commits`를 돌리면 실패한다.
