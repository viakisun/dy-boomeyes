...
# ADR-009 — PWA 알림 (Accepted 2026-09-06 — B13)
...
## Rules
- [x] 앱은 `new Notification`을 직접 쓰지 않는다(dev 폴백 제외) — 표시는 SW 경로.
- [x] 권한은 사용자 행동(종 아이콘 → 시트)에서만 요청한다. 자동 프롬프트 금지.
- [x] 딥링크 URL 규칙은 `toPushPayload()` 단위 테스트로 고정한다.

사용자 결정 필요: 목업 수준(로컬 알림) 승인 · 종 아이콘을 셸 앱바에 두는 것(PWA 캡처 전량 갱신).

결정(2026-09-06, 사용자): 승인 — 로컬 알림 수준 · 종 아이콘은 셸 앱바(PWA 캡처 전량 갱신 수용). B13 착수.
