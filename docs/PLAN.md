...
기준일 2026-09-05. 웨이브 = 2주. 전략 근거: `METHOD.md` · 범위 원천: `ssot/`(화면 65 · FR 44 · DISC 60 — 미결 53). 계약 기준선(9/10 동결 · 9/30 시연 · 12/31)은 `ssot/meta.yaml`에 그대로 두고, 여기서 재기준선을 관리한다 — DY 합의 전까지 "제안".
...
| **W2.6 브랜드·삽화·격차 보강**(사용자 요청 2026-09-07 · 계획 `~/.claude/plans/form-tingly-gray.md`) | 완료(2026-09-07) — PR D #32 · L #33 · M #34 · G1 #35 · G2 #36 · G3 #37: D `docs/deck-disc`(DISC-047~054 · DISC-005/007/008/009/039/040 갱신 · 용어 3) → L `feat/brand-logo`(Logo mark/glyph/lockup · 사이드바 접힘 글리프 · 로그인 두 톤 · favicon·PWA 아이콘 빌드 · DY-design §13) → M `feat/media-illustrations`(참고자료 v5.0 삽화 → 타일 스틸·루프 MP4 · bbox 정정 · SW 선캐시 mp4 제외) → G1 영상 확보 상태·카메라 장착 위치·설치 구성도 → G2 미연동·미수신 표기 규칙 → G3 AI 판단 유보·현장 신고(FR 신규 제안 · kind proposed). 덱 22토픽 판정: 채택 4 · DISC 8 등록 · 기각 2(카메라 후보 그대로 · 3.6GB/일 수치) · 제조사 사진 제외(저작권) |
| **W2.7 워크플로우 카탈로그**(사용자 요청 2026-09-07 · 계획 `~/.claude/plans/form-tingly-gray.md`) | 진행 — `ssot/scenarios.yaml workflows[]` 18건(장비 이상 3 · 영상·AI 6 · 사람·현장 2 · 부품 2 · 서류 1 · 운영·계약 4) · 스키마·참조/역할/어휘 검사·생성 표 `docs/generated/WORKFLOWS.md` · 안내 `docs/WORKFLOWS.md`. 상태 = 화면으로 재현 14 · 상태 데이터 필요 2 · 제안 2. 시연 대본 10장면은 그대로 둔다. 드러난 빈칸 4건(알림 3종 수신 역할 미정 · 기한 초과 통보 미구현 · 정비 조치 보고 채널 · 임대 만료 알림 부재)은 표기만 |
...
### 소유주 데모 개선 — 실행 계획 작성(2026-09-11)

- 사용자 요청: 페이지별 디자인·KPI·텍스트·소유주 진입 개선, 소유주 모의 의견 반영, AI 개발과 전체 검증 체계 수립.
- [상세 실행 계획](plans/owner-demo-implementation-plan.md): OD-00~14, 논리 화면 7종의 WEB/A4 PWA 구현, AC-O01~18, VT-01~12, 단계 게이트 G0~G8.
...
- 2026-09-12 후속: 운영 현황을 드릴다운 관제(전국 → 현장 → 호기)로 재구성 — 고객 V5 `[구현 예시]` 3장 근거, 사용자 결정(120대·현장 13 시연 기본 · KPI 카드 대신 상태 띠 · 패널 안 실시간 영상 · PWA 지도 전면 + 시트 3단 · 시뮬레이터 on/off). 체인 `feat/owner-sites-seed` → `feat/map-camera-kinds` → `feat/owner-overview-drilldown` → `feat/owner-map-sheet-pwa` → `feat/owner-sim` → `docs/owner-drilldown-evidence`(reviewer 5건 "머지 가능"). 이후 3페이지 디자인 리뷰 4건 반영(`feat/owner-overview-regions` 6d5665a · `feat/owner-overview-polish` 7825f5d · 증거 재생성 `docs/owner-drilldown-evidence-2`, reviewer 머지 가능). 캡처 144조합 · 예외 30건 · owner e2e 108(소유주 스펙) · 전체 관련 e2e 151. 문서 [디자인 언어 §10](design/OWNER-VISUAL-LANGUAGE-2026-09-11.md).
- 2026-09-13 후속: Claude Design 시안(«운영 현황 목업» · «소유주 업무 기획» · «보유 장비 기획»)을 새 기준으로 소유주 앱을 재구성. 사용자 결정 — 시안만 기준 · 토큰 전면 전환 · accent를 파랑 쪽으로 · 정상 초록은 시안 그대로(원칙 4 개정) · 모바일은 보유 장비 카드 유지 / 카메라 2×3 / 계약 두 단계. 단계 P1~P9: 디자인 언어 전환(ADR-014) → 원천 해제(owner_demo 규칙 완화 · `owner:counts` 게이트 신설) → 셸·종 알림 → 자료 축(카메라 · 지표 · 소모품 · 서류 · 요청 · 운전자) → 운영 현황 3단계(카메라 벽 · 타일 없는 국경 지도 · 회색조 현장 타일 · 전체 화면) → 보유 장비 표 → 계약(요청 → 배정) → 운전자(명단 · 서류) → 증거·문서. 좌측 메뉴는 시안대로 운영 현황 · 보유 장비 · 계약 셋이고 알림은 헤더 종이다. 브랜치 `feat/owner-overview-stages`(미머지 — reviewer 판정 대기). 문서 [디자인 언어 §11](design/OWNER-VISUAL-LANGUAGE-2026-09-11.md).
- 2026-09-13 후속 2: 시안 원본과 구현을 화면별로 대조해 어긋난 여덟 곳을 맞췄다(브랜치 `feat/owner-design-parity`, 미머지 — reviewer 판정 대기). 사용자 결정 — 전국 마커는 시안 문법 전면 채택 · 정보 카드는 세 단계 모두 좌측. R1 패널 좌측·띠 숫자 먼저 → R2 전국 카드를 현장 단위로 → R3 전국 지도를 현장 원으로(지역 집계와 지역 단계 삭제 · `spreadCircles` 중심 거리 밀어내기 · 이름표 여섯 후보, 자리가 없으면 숨김) → R4 보유 장비 상태 요약·여섯 열·계약 목록의 지역 → R5 레일의 새 요청 배지 → R6 증거·문서. 증거 [owner-implementation-7bf66c8d](design/evidence/owner-implementation-7bf66c8d-2026-09-13T15-03-45.140Z/manifest.json) 캡처 208 · 예외 38건/46프레임 · 성능 통과 · owner e2e 124 · owner:check 0 · 검토안 26쪽(시각 검수 pass · 고객 확인 별도). 문서 [디자인 언어 §11](design/OWNER-VISUAL-LANGUAGE-2026-09-11.md).
- 2026-09-14 후속: **호기 화면을 하나로** — 사용자 지적(「시안은 전체 지도 → 현장 지도 → 호기 상세인데 옛 호기 상세만 있다」)에서 출발해 옛 독립 상세 화면 둘을 폐기하고 현황의 호기 단계로 합쳤다. S1 없앨 화면에만 있던 것을 옮기고(마지막 수신값 · 이상·점검 이력 · 현장 영상 · 목록 복귀) → S2 목록·알림·운전자·서류·영상의 길을 모두 호기 단계로 → S3 원천·화면 삭제(요구 13건은 지우지 않고 호기 단계로 교체) → S4 도구와 검토안(본문이 전국 한 장뿐이라 문서만 보면 드릴다운이 없는 것처럼 읽혔다 — 전국·현장·호기 세 쪽으로 폈다) → S5 문서·증거. 브랜치 `feat/owner-design-parity`(미머지). 문서 [디자인 언어 §11](design/OWNER-VISUAL-LANGUAGE-2026-09-11.md).
...
| **W4~W5 2단계** | 11/3~11/28 | wave 4 화면 17: A4 소유주 데모 7종(진입·현황·보유·호기·영상·서류·이상) · B3 전체(B3-07 포함) · 현장 프로파일 8축(FR-029) · 저장 영상 타임라인(IF-007/008) · 바디캠·동의 스텁(FR-030/031) · 부품 생애주기 실동작(FR-032·035) · 이벤트 복기(FR-033) · **wave 5 화면 7: 계약·운전자**(B1-13·14·15 · A4-03·04·05·06 — 시안 채택으로 2026-09-12 편입, specs/owner-contracts·owner-drivers) | 65화면 전부 실 또는 mock 어댑터 | OEM 부품 기준(DISC-038) · 태그 방식(DISC-043) · 요청 인입 경로(DISC-052) |
