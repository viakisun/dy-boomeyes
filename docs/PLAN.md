...
| 다크 렌더(shell-auth AC-6) — 완료(`?theme=` · 웹 토글 · `capture --dark` · e2e 6) · 시각 회귀 기준선(0.2%) · demo-scripts 장면 1~8 · PWA manifest/SW · `maintenance` 로그인 경로 · tokens:lint 잔여(border-N · ring-N · duration-N · max-w-*) · 카탈로그 밖 컴포넌트 검사 · PWA 측 AX-1 픽스처 | 진행 예정 |
