...
- **2026-09-05 DemoClock 점프가 invalidateAll에 지워짐(W1)** — `bootMock`이 live 진입마다 `clock.reset()`을 불러 "1시간 경과" 오프셋이 다음 load에서 사라졌다. 절차: 화면 진입은 고정 해제(`unfreeze`)만 · 시각 리셋은 `resetMock`(로그아웃)에서만 · 시간 점프 기능은 e2e로 "점프 → 다시 읽기"까지 검사.
