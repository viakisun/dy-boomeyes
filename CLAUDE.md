...
- **2026-09-05 상대 경로 셸의 오프라인 부팅 실패(W1)** — 서비스 워커가 캐시한 셸(`/`)을 `/a2/login`에서 오프라인으로 열자 자산이 `./_app/…` 상대 경로라 `/a2/_app/…`으로 풀려 부팅이 실패했다(SvelteKit `paths.relative` 기본 true · 프리뷰 서버가 `/`를 상대 경로로 렌더). 절차: PWA는 `paths.relative: false` · SW 오프라인 검증은 e2e(`pwa-install`: SW 제어 → offline → reload → 셸)로 · "빈 흰 화면"은 `page.on('requestfailed')`·`pageerror`를 찍는 프로브부터.
