...
- **2026-09-07 체인 중간의 `;`와 백그라운드 cwd(W2.5 D9)** — 편집 스크립트가 assert로 죽었는데 게이트 뒤 보고용 `; tail log;`가 체인을 끊어 검증 없이 커밋됐다(메시지가 주장한 편집 7건 중 4건만 반영). 같은 날 병렬 포그라운드 `cd`가 세션 cwd를 바꿔 백그라운드 체인이 main 저장소에서 `git add`를 시도했다(pathspec 실패로 무해). 절차: 체인에는 `;`를 한 번도 쓰지 않는다 — 보고용 tail은 체인이 끝난 뒤 별도 명령 · **모든** 명령(포그라운드 포함)을 `cd <절대경로> &&`로 시작한다 — 세션 cwd는 백그라운드 명령이 실패한 뒤 프로젝트 루트로 리셋될 수 있다(G1에서 편집·verify가 main 체크아웃에서 돌았음) · 편집 전 `git branch --show-current` 확인 · 커밋 전 `git show --stat`의 파일 목록을 커밋 메시지의 주장과 대조.
- **2026-09-11 `set -e` 스크립트의 `게이트 && echo OK`(DY gap 라운드, #63)** — bash `set -e`는 `&&`/`||` 리스트에서 마지막이 아닌 명령의 실패를 무시한다. `pnpm verify > log && echo VERIFY:0`은 verify가 빨간데도 스크립트가 계속 진행돼 CI에서만 잡혔다(session.ts 중복 import). 절차: 체인 스크립트의 게이트는 **단독 문장**(`pnpm verify > log` 다음 줄 `echo VERIFY:0`) 또는 `cmd || exit 1` · 알림 출력에 `VERIFY:0`이 없으면 실패로 읽는다(있을 때만 성공) · 인터랙티브 `&&` 체인 규칙(위)과 같은 뜻이다 — 스크립트로 옮길 때 형태가 바뀌면 의미도 바뀐다.
- **2026-09-11 체인 PR 자동 병합의 중복 추가(#63)** — base PR(#62)이 리뷰 뒤 수정 커밋을 받고 squash 머지되면, 그 위에 만든 체인 PR은 base의 **수정 전** 파일을 물려받아 3-way 병합에서 양쪽이 같은 파일의 다른 hunk에 넣은 줄이 **둘 다** 살아남는다(`session.ts` `import type { Scope }` 1행+4행 → TS2300). 충돌이 안 났으므로 `--ours/--theirs` 레시피로는 안 잡힌다. 절차: 체인 PR을 main으로 옮긴 뒤 `git diff origin/main -- <base PR이 고친 파일들>`이 비었는지 확인 · `pnpm check`를 병합 직후에 돌린다 · base PR이 수정 커밋을 받으면 체인 PR에 즉시 병합한다.
- **2026-09-12 파생 수치를 문장에 적는 습관(소유주 원천 해제 · 리뷰 6라운드)** — 소유주 화면 목적을 7 → 11로 늘리자 그 수를 문장·단언에 적어 둔 곳이 도구·문서 11곳에서 차례로 드러났다. 리뷰가 여섯 번 「머지 불가」를 냈고 매 라운드 **새 인스턴스**가 나왔다(도구 5곳 → 리터럴 14 → 산출물 텍스트 3곳 → 인접 파일 3곳 → 내가 만든 동어반복 → 문서 2곳). `pnpm verify`는 이 계열을 하나도 잡지 못한다 — 생성물이 최신인지만 보고, 문장의 수나 verify 체인 밖 도구(`tools/owner/*` · `tools/capture/owner*`)는 보지 않는다. 절차: 원천의 개수를 바꾸면 **그 원천을 읽는 도구를 직접 다 실행한다**(verify 녹색은 근거가 아니다) · 개수는 원천 한 곳에만 두고 도구·문서는 파생하거나 「`--list`가 알려 준다」로 미룬다 · 개수 단언을 지울 때는 그것이 무엇을 잡고 있었는지 먼저 적고 실체 단언으로 갈아탄다(`all.length !== CASES.length * 2`처럼 정의상 참인 비교로 바꾸면 보호를 잃는다) · 새 게이트는 통과가 아니라 **일부러 깨서 발화**를 확인한다 · 수치가 남아도 되는 곳은 날짜 박힌 기록물(`docs/plans/*` · `docs/design/evidence/*` · `REVIEW-*` · PLAN의 「YYYY-MM-DD 후속」 일지)뿐이고, 명세·README·게이트 문서처럼 「현재 이렇다」를 말하는 글에는 적지 않는다.
- **2026-09-12 무거운 게이트의 동시 실행(소유주 리뷰 9차)** — 리뷰 에이전트가 `pnpm verify`에서 600초 멈춰 실패했고, 메인 세션의 verify도 두 번 600초를 넘겼다. 단독 실행으로 재보니 **19.65초**다(단계별: ssot 0.4 · tokens 0.3+0.3 · owner:counts 0.2 · lint 8.6 · check 7.2 · test 1.8). 즉 저장소 문제가 아니라 여러 에이전트가 같은 무거운 명령(`turbo run check`의 svelte-check · `turbo run test`, `turbo.json`은 `cache: false`)을 동시에 돌린 경합이다. 절차: 리뷰 에이전트에게는 **`pnpm verify`를 금지하고 빠른 부분만** 주라(`ssot:check`(+`--specs` `--docs` `--commits`) · `tokens:check` · `tokens:lint` · `owner:counts` · `*.mjs --list` — 각 1초 미만) · 메인 세션의 게이트 결과를 프롬프트에 인용해 재실행 이유를 없애라 · 판정은 코드 읽기로 하고 실행은 보조로 쓰라고 명시하라 · 에이전트가 600초 멈추면 포트·잔여 프로세스를 확인하고(`lsof -nP -iTCP:4173 -iTCP:4174 -sTCP:LISTEN` · `pgrep -fl "vitest|turbo|playwright|svelte-check"`) 실행 금지 목록을 넓혀 새 에이전트로 재리뷰한다.
