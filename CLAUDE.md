...
- **2026-09-07 체인 중간의 `;`와 백그라운드 cwd(W2.5 D9)** — 편집 스크립트가 assert로 죽었는데 게이트 뒤 보고용 `; tail log;`가 체인을 끊어 검증 없이 커밋됐다(메시지가 주장한 편집 7건 중 4건만 반영). 같은 날 병렬 포그라운드 `cd`가 세션 cwd를 바꿔 백그라운드 체인이 main 저장소에서 `git add`를 시도했다(pathspec 실패로 무해). 절차: 체인에는 `;`를 한 번도 쓰지 않는다 — 보고용 tail은 체인이 끝난 뒤 별도 명령 · 백그라운드 체인은 `cd <절대경로> &&`로 시작 · 커밋 전 `git show --stat`의 파일 목록을 커밋 메시지의 주장과 대조.
