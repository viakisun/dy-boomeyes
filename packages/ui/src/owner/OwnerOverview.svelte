...
<div class="gap-stack-xl flex min-w-0 flex-col">
  <div class="gap-inline-lg flex flex-wrap items-end justify-between">
    ...
    <p class="text-body-sm text-fg-muted">{fmtDateTime(data.at)} 기준</p>
  </div>
  <form
  ...
  >
    ...
    <button
      type="submit"
      class="{ownerControl()} rounded-control bg-accent text-accent-on-solid hover:bg-accent-solid-hover px-inset-lg text-body-md shrink-0"
      >검색</button
    >
  </form>
  ...
  <div class="gap-stack-xl grid min-w-0 grid-cols-1 xl:grid-cols-2">
    <section class="gap-stack-md flex min-w-0 flex-col" aria-labelledby="owner-attention-title">
      <div class="gap-inline-sm flex flex-wrap items-center justify-between">
        ...
        <a href={ownerHref(url, 'alerts', app)} class={ownerLink()}
      ...
      {#if shown.length > 0}
          ...
          {#each shown as alert (alert.id)}
            ...
            <li>
              <a
                ...
                class="{ownerControl()} gap-inline-md px-inset-lg py-inset-lg hover:bg-ui-hover flex min-w-0 items-start"
              ...
              >
                ...
                <div class="gap-stack-xs flex min-w-0 flex-1 flex-col">
                  ...
                  <p class="gap-inline-md text-body-sm text-fg-muted flex flex-wrap">
                    <span>{fmtDateTime(alert.at)}</span><span class={alert.read ? '' : 'text-fg font-semibold'}
                      >{alert.read ? '읽음' : '미확인'}</span
                    >
                  </p>
                </div>
        ...
        <p class="text-body-sm text-fg-muted">
          전체 알림 {summary.alerts}건 중 {shown.length}건 표시
        </p>
      {:else}
        <EmptyState
    ...
    <section class="gap-stack-md flex min-w-0 flex-col" aria-labelledby="owner-location-title">
      <div class="gap-inline-sm flex flex-wrap items-center justify-between">
        ...
        <a href={ownerHref(url, 'fleet', app)} class={ownerLink()}
      ...
      {#if devices.length > 0}
        ...
        >
          {#each devices as device (device.id)}
            <li><EquipmentRow {device} href={ownerHref(url, 'detail', app, {}, device.id)} location={!map} /></li>
          {/each}
        </ul>
        <p class="text-body-sm text-fg-muted">
          전체 {data.devices.length}대 중 {devices.length}대 표시
        </p>
      {:else}
        <EmptyState
          title="표시할 현장이 없습니다"
