...
const LADDER = {
  light: { L: [0.995, 0.982, 0.958, 0.93, 0.897, 0.857, 0.79, 0.64, 0.5, 0.45, 0.42, 0.27], C: [0.04, 0.09, 0.18, 0.27, 0.36, 0.46, 0.58, 0.75, 1, 1, 0.85, 0.5] },
  dark: { L: [0.17, 0.2, 0.24, 0.28, 0.32, 0.36, 0.42, 0.48, 0.52, 0.58, 0.8, 0.95], C: [0.15, 0.2, 0.3, 0.4, 0.5, 0.55, 0.6, 0.7, 1, 1, 0.75, 0.3] },
};
const NEUTRAL_L = {
...
export function buildRamps(config, brand) {
  const hues = config.hues.map((h) => (h.role === 'accent' && brand.accent ? { ...h, name: brand.accent.hue ?? h.name, anchor: brand.accent.anchor ?? h.anchor, pins: brand.accent.pins ?? h.pins, ladder: brand.accent.ladder ?? h.ladder } : h));
  const out = { light: {}, dark: {}, onSolid: { light: {}, dark: {} }, meta: {} };
  const byName = Object.fromEntries(hues.map((h) => [h.name, h]));
  for (const hue of hues) {
    const src = hue.hueFrom ? (hue.hueFrom === 'accent' ? hues.find((h) => h.role === 'accent') : byName[hue.hueFrom]) : hue;
  ...
  for (const theme of THEMES) for (const [name, ramp] of Object.entries(out[theme])) {
