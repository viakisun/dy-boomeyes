...
대상: B0 로그인 · B1 관제 · B2 본사 · B3 현장 · B4 관리자(백오피스). 기본 모드 light/compact. 모니터링 월보드·쇼케이스(B1-07 · B3-06 · CameraWall)만 dark 강제.
...
8. **다크(월보드).** `CameraWall`·쇼케이스는 컴포넌트 루트에 `data-theme="dark"`를 두고 내부만 다크. 큰 수치 display-lg · 상태 solid 9단 · 텍스트 fg.default.
