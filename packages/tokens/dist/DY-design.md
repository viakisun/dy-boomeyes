...
| **OwnerDrivers** | both | 시안 «소유주 업무 기획» 운전자 · Trackunit Operators | 명단 · 서류(같은 화면의 두 갈래) | default/empty/expiring(자격 만료 임박)/missing(서류 미비) | 운전자는 소유주 소속이고 배정이 매일 바뀐다(FR-027 · «확정 2026-09-12»). 서류는 사람 4종 — 차량 6종과 다른 갈래다. 새 메뉴를 만들지 않고 계약 옆에서 연다. |
...
| **CameraViewer** | both | 시안 «운영 현황 목업» 전체 화면 · Motive Fleet View Live | 단일 카메라 + 필름 띠 | 닫힘/열림 | 카메라 타일을 누르면 열리는 전체 화면(«확정 2026-09-12»). <dialog> 모달이라 Esc·포커스 가둠·최상위 레이어가 딸려 온다 — z 유틸리티를 쓰지 않는다. ←·→로 카메라를 옮긴다. |
...
| **OwnerLiveTile** | both | Motive Fleet View Live · V5 관제기능 | capture(정지)/live(자동 재생) · fill(전체 화면) | default | 호기 패널·카메라 벽의 실시간 영상 타일 — 6초 샘플 루프(muted·loop·playsinline), 시연 클립 캡션, 해제 시 pause·src 제거. fill이면 영역을 채우고(전체 화면) 영상은 레터박스 · 안내를 길게 밝힌다. |
...
§11.7을 따른다(웹도 comfortable 밀도 · 목록은 List + 정렬 열 행).
...
4. **시각 인코딩** — 계약 기간 = PeriodBar(양끝 time · 오늘 마커 `radius.mark` · 종료까지 D-n, ≤ 30일 warning), 보유 구성 = FleetSummary 분포 막대(`role=img`), 담당자 = ContactCard(이니셜 아바타 · tel: 링크 · 복사), 장비 = 카메라 poster 스틸 또는 IconTile, 서류 = previewUrl 썸네일 또는 IconTile, 위치 = 지도 스니펫(장면 카메라 `camera` — 전국 maxZoom 7 · 지역 10 · 현장 17 · 상세 1대 16, 지명은 `labelLocale="ko"`, 마커 문법: 정상 현장·지역 = 작은 중립 점(보관소는 사각 `radius.mark`) · 이상 = 상태색 원 + 글리프 3종(✕ · ! · 수신 없음 아이콘) · 호기 핀은 ✓/글리프, 480px 미만은 번호 원형 · 현장·지역 알약은 대수 배지의 무게(≥ 10 mid · ≥ 20 heavy)로 규모를 드러낸다 · 리더선은 호기 핀 가로 펼침에만 · 겹침은 `resolveOverlaps` 2축 밀기 · 베이스맵은 테마를 따르고(Positron/Dark Matter) 알약 지도에서는 지명을 감쇠한다).
   **드릴다운(현황)** — 전국(지역 알약 7 · 누르면 그 지역 현장 알약) → 현장(호기 핀) → 호기(패널: 실시간 영상 타일 · 설치/완료/임대기간 · 담당자 · 전압/단선/고장코드 · 관련 서류 · 상세). URL(`?site=`·`?device=`)이 단계, 경로는 OverviewCrumbs, 상태 띠(StatusStrip)는 KPI 카드가 아니라 칩 한 줄. 활동 시뮬레이터(셸 토글 · `?sim=`)는 기본 off.
...
| — · 빈 값(연동 안 된 계측) | 미연동 |
| 옛 값 그대로(수신 임계 초과) | 미수신 · 마지막 HH:MM |
