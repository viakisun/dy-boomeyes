...
4. **시각 인코딩** — 계약 기간 = PeriodBar(양끝 time · 오늘 마커 `radius.mark` · 종료까지 D-n, ≤ 30일 warning), 보유 구성 = FleetSummary 분포 막대(`role=img`), 담당자 = ContactCard(이니셜 아바타 · tel: 링크 · 복사), 장비 = 카메라 poster 스틸 또는 IconTile, 서류 = previewUrl 썸네일 또는 IconTile, 위치 = 지도 스니펫(장면 카메라 `camera` — 전국 maxZoom 7 · 지역 10 · 현장 17 · 상세 1대 16, 지명은 `labelLocale="ko"`, 핀은 상태 점 + 호기 알약 · 480px 미만은 번호 원형 핀 · 현장 알약(짧은 이름 + 대수) · 지역 원(대수), 겹침은 `resolveOverlaps`로 2축 밀기).
   **드릴다운(현황)** — 전국(현장 알약 13 · 가시 폭 760px 미만은 지역 원 7) → 현장(호기 핀) → 호기(패널: 실시간 영상 타일 · 설치/완료/임대기간 · 담당자 · 전압/단선/고장코드 · 관련 서류 · 상세). URL(`?site=`·`?device=`)이 단계, 경로는 OverviewCrumbs, 상태 띠(StatusStrip)는 KPI 카드가 아니라 칩 한 줄. 활동 시뮬레이터(셸 토글 · `?sim=`)는 기본 off.
