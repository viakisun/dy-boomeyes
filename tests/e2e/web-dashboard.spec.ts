...
test('[B1-02] 소유주 owner01 일반 세션 — 운영 현황 · 보유 장비 120대 · 소유주 메뉴 3개 + 알림 종 [FR-001] [FR-002]', async ({
  page,
}) => {
  await page.goto('/login');
  ...
  // 메뉴는 운영 현황 · 보유 장비 · 계약 셋 — 이상·점검과 장비 서류는 헤더 종과 호기 화면으로 내려갔다(시안 «결정 2026-09-12»)
  await expect(navigation.getByRole('link')).toHaveCount(3);
  for (const label of ['운영 현황', '보유 장비', '계약'])
