// 건설사 앱이 서버에서 받아 둔 것과 지금 보고 있는 것.
// 소유주와 달리 자기 현장만 본다 — 전국 지도도, 보유 장비도, 요청 배정도 없다.
import * as api from '../../shared/api';
import type { Alert, Site, Unit } from '../../shared/types';

/** 로그인이 생기기 전까지는 어느 건설사로 볼지 주소로 고른다(?builder=한빛건설). */
export const currentBuilder = () => new URLSearchParams(location.search).get('builder') ?? '한빛건설';
...
export async function loadEverything() {
  ...
  server.sites = sites.filter(s => s.builder === builder);
