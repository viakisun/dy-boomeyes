...
function render() {
  ...
  $('#nbadge').textContent =
    inboxEvents().filter(e => !state.read.has(e.title + (e.u ? e.u.code : e.s.id))).length || '';
  if (req) {
    $('#band').classList.add('hide');
    $('#wall').hidden = true;
    document.body.className = '';
    $('#crumb').innerHTML = state.req
      ? `<button onclick="state.req=null;render()">요청</button><span class="sep">›</span><b>${state.req.site}</b>`
      : '<b>요청</b>';
    renderReq();
    return;
  }
  ...
  if (state.level === 'nation') {
    ...
    siteLayer.addTo(map);
    const s = state.site,
      b = L.latLngBounds(s.units.map(u => ll(s, u.d)));
    ...
    const free = {
      x: left + 30,
      y: state.level === 'unit' ? 30 : 90,
      w: mapEl.clientWidth - left - 70,
      h: mapEl.clientHeight - (state.level === 'unit' ? 60 : 130),
    };
    const p = map.latLngToContainerPoint(focus);
    map.panBy([p.x - (free.x + free.w / 2) + (state.level === 'unit' ? -70 : 0), p.y - (free.y + free.h / 2)], {
      animate: false,
    });
    document
      .querySelectorAll('.um')
      .forEach(el => el.classList.toggle('on', state.unit && (el as any).dataset.unit === String(state.unit.num)));
  }
}
...
const feedHtml = (u, i) => {
  ...
  const bg = `background-image:url(screens/cpb-scene.png);background-size:${CAMVIEW[i]}`;
  return (
    `<div class="feed ${c[0]} ${ev ? 'alert' : ''}" data-cam="${i}" style="${bg}" onclick="openFull(${i})">` +
...
function renderWall() {
  const u = state.unit;
  $('#wall').innerHTML = u.cams.map((c, i) => feedHtml(u, i)).join('');
  fitWall();
}
...
function fitWall() {
  const w = $('#wall');
  if (w.hidden) return;
  const n = state.unit.cams.length,
    gap = 12,
    W = w.clientWidth,
    H = w.clientHeight;
  let best = null;
...
function renderFull() {
  ...
  const thumb = (cc, k) =>
    ...
    `style="background-image:url(screens/cpb-scene.png);background-size:${CAMVIEW[k]}" ` +
...
document.addEventListener('keydown', e => {
  if (state.full == null) return;
  if (e.key === 'Escape') closeFull();
  if (e.key === 'ArrowLeft') openFull((state.full + state.unit.cams.length - 1) % state.unit.cams.length);
  if (e.key === 'ArrowRight') openFull((state.full + 1) % state.unit.cams.length);
});
  ...
  if (s && state.level !== 'nation')
    h +=
      state.level === 'site'
        ...
        : `<span class="sep">›</span><button onclick="go('site')">${s.name}</button>`;
...
function renderBand() {
  ...
  const cells = [
    [
      'total',
      units.length,
      state.level === 'nation' ? '보유 장비' : state.site.st === 'store' ? '보관 장비' : '투입 호기',
    ],
    ['', c('run'), '가동 중'],
    ['fault', c('fault'), '고장'],
    ['check', c('check'), '점검'],
...
function inboxEvents() {
  ...
  for (const s of SITES)
    for (const u of s.units) {
      ...
      if (u.st === 'late')
        ev.push({ k: 'late', t: '7. 3. 08:22', s, u, title: '수신 지연', sub: '마지막 수신 이후 응답 없음' });
      if (u.st === 'check')
        ev.push({ k: 'check', t: '어제', s, u, title: '점검 시기', sub: UNIT_MSG.check + ' · 기한 9. 20.' });
      for (const e of (u.ai || []).filter(e => e.lvl === 'warn'))
        ev.push({ k: 'ai', t: e.t.slice(0, 5), s, u, title: 'AI · ' + e.type, sub: e.detail });
      for (const p of u.parts)
        if (p[1] === 'wear' && p[2] >= p[3])
          ev.push({ k: 'check', t: '오늘', s, u, title: '소모품 한계', sub: `${p[0]} ${p[2]}% · 교체 기준 ${p[3]}%` });
    }
  for (const s of SITES)
    if (s.st !== 'store' && s.dday <= 35)
      ev.push({
        ...
        u: null,
        title: '계약 종료 임박',
        sub: `${s.name} · ${s.units.length}대 · ${s.end}`,
      });
  return ev;
}
state.read = new Set();
...
function renderInbox() {
  const ev = inboxEvents(),
    unread = ev.filter(e => !state.read.has(e.title + (e.u ? e.u.code : e.s.id)));
  $('#inbox').innerHTML =
    `<header><h2>알림<small>미확인 ${unread.length}</small></h2><button onclick="inboxEvents().forEach(e=>state.read.add(e.title+(e.u?e.u.code:e.s.id)));renderInbox();render()">모두 읽음</button></header>
    <div class="list">${ev
      .map((e, i) => {
        const id = e.title + (e.u ? e.u.code : e.s.id);
        const site = `SITES.find(x=>x.id==='${e.s.id}')`;
        const jump = e.u ? `go('unit', ${site}, ${site}.units.find(x=>x.num===${e.u.num}))` : `go('site', ${site})`;
        const mark = `state.read.add('${id.replace(/'/g, '')}');toggleInbox(false);`;
        return `<div class="ev ${e.k} ${state.read.has(id) ? 'read' : ''}" onclick="${mark}${jump}">
      <span class="d"></span><div><div class="nm">${e.u ? e.u.num + '호기 ' : ''}<span>${e.title}</span></div><div class="sub">${e.u ? e.s.name + ' · ' : ''}${e.sub}</div></div><span class="t">${e.t}</span></div>`;
      })
      .join('')}</div>
...
function renderReq() {
  ...
  if (!state.req) {
    ...
    const countSt = st => REQS.filter(r => r.st === st).length;
    const stored = SITES.find(s => s.st === 'store').units.length;
    const endingSoon = SITES.filter(s => s.st !== 'store' && s.dday <= 90).reduce((a, s) => a + s.units.length, 0);
    el.innerHTML =
      `
      <div class="ftop"><h2 class="rh">현장 요청</h2><span class="rsub">현장 안전관리자가 보낸 CPB 요청을 받아 호기를 배정합니다</span></div>
      <div class="fmeta"><span><b>${countSt('new')}건</b> 새 요청 · 배정 중 <b>${countSt('assign')}건</b></span>` +
      ...
      <div class="ftbl"><table>
        ...
        <tbody>${rows
          .map(
            r => `<tr class="u" onclick="state.req=REQS.find(x=>x.id==='${r.id}');render()">
  ...
  const nStored = cands.filter(c => c.s.st === 'store').length;
  const why = ({ u, s }) =>
    s.st === 'store'
  ...
  el.innerHTML =
        ...
        <div class="rsec pick">
          ...
          ${
            picked.length
              ? `<div class="pl">${picked
                  .map(k => {
                    const c = cands.find(x => x.u.code === k);
                    return `<div>${c.u.num}호기 <span class="mono-s">${c.u.code}</span><button onclick="togglePick('${k}')">×</button></div>`;
                  })
                  .join('')}</div>`
    ...
    `
        <div class="ftbl"><table>
          ...
          <tbody>${cands
            .map(
              c => `<tr class="${picked.includes(c.u.code) ? 'sel' : ''}">
            ...
            <td>${risk(c)}</td>
            <td class="r"><button class="pickb ${picked.includes(c.u.code) ? 'on' : ''}" onclick="togglePick('${c.u.code}')">${picked.includes(c.u.code) ? '배정됨' : '배정'}</button></td></tr>`,
            )
            .join('')}</tbody></table></div>
      </div>
    </div>`;
...
function togglePick(code) {
  ...
  if (i >= 0) p.splice(i, 1);
  else if (p.length < r.n) p.push(code);
  renderReq();
}
...
function renderFleet() {
  ...
  let rows = all.filter(({ u, s }) => {
    ...
    if (state.fst === 'attn' ? !['fault', 'check', 'late'].includes(u.st) : state.fst !== 'all' && u.st !== state.fst)
      return false;
  ...
  const attnN = { fault: 0, check: 0, late: 0 };
  all.forEach(({ u }) => {
    if (attnN[u.st] != null) attnN[u.st]++;
  });
  ...
  const th = (k, label, cls?) =>
    `<th class="${cls || ''} ${state.sort === k ? 'on' : ''}" onclick="state.sort='${k}';renderFleet()">${label}</th>`;
  ...
  if (!rows.length) body = `<tr><td colspan="6" class="empty">조건에 맞는 장비가 없습니다</td></tr>`;
    ...
    body = [...bySite.entries()]
      .map(
        ([id, rs]) =>
          `<tr class="g"><td colspan="5">${rs[0].s.name}<span>${rs.length}대 · ${rs[0].s.st === 'store' ? '보관소' : 'D-' + rs[0].s.dday}</span></td></tr>` +
  ...
  el.innerHTML =
    ...
    `oninput="state.q=this.value;renderFleet();document.getElementById('fq').focus()"></label>` +
    `
      <select onchange="state.fst=this.value;renderFleet()">${[
        ['all', '상태 전체'],
        ['attn', '확인 필요'],
        ['fault', '고장'],
        ['check', '점검'],
        ...
        .map(([v, l]) => `<option value="${v}" ${state.fst === v ? 'selected' : ''}>${l}</option>`)
        .join('')}</select>
      ...
      <select onchange="state.fexp=this.value;renderFleet()">${[
        ['all', '계약 종료 전체'],
        ['30', '30일 이내'],
        ['60', '60일 이내'],
        ['90', '90일 이내'],
      ]
        .map(([v, l]) => `<option value="${v}" ${state.fexp === v ? 'selected' : ''}>${l}</option>`)
        .join('')}</select>
    ...
    `<span class="fault">고장 <b>${attnN.fault}</b></span>` +
    `<span class="check">점검 <b>${attnN.check}</b></span>` +
    `<span class="late">지연 <b>${attnN.late}</b></span>` +
...
function renderCard() {
  ...
  if (state.level === 'nation') {
    ...
    const warnUnits = SITES.flatMap(s => s.units).filter(u => warnSt.includes(u.st)).length;
    card.innerHTML =
      `<header><h2>확인 필요<small>${warn.length}개 현장 · ${warnUnits}대</small></h2>` +
      `<a href="#" onclick="goTab('fleet');return false">전체 ${SITES.length}개 현장</a></header>` +
      `
      <div class="body">${warn
        .map(
          s => `<div class="row" data-site="${s.id}" onclick="go('site', SITES.find(x=>x.id==='${s.id}'))">
        <div><div class="nm">${s.name}</div><div class="sub"><em style="color:${COLOR[s.st]}">${LABEL[s.st]}</em> · ${s.note}</div></div><div class="cnt">${s.n}대</div></div>`,
        )
        .join('')}</div>`;
    return;
  }
  ...
  // 복사 버튼: 누르면 1.2초간 ✓로 바뀐다
  const copyTel = `navigator.clipboard.writeText('${s.tel}');this.textContent='✓';setTimeout(()=>this.textContent='⧉',1200)`;
  const contract =
    s.st === 'store'
      ? ''
      ...
      <div class="sec" style="border-bottom:0">
        ...
        <div class="call"><button onclick="location.href='tel:${s.tel}'">☏ ${s.mgr} · ${s.tel}</button>` +
        `<button class="ic" title="복사" onclick="${copyTel}">⧉</button></div>` +
        `
      </div></details>`;
  if (state.level === 'site') {
    const order = { fault: 0, check: 1, late: 2, run: 3, store: 4 };
...
function unitCard(u, s, warn, since) {
  ...
  // 가장 급한 부품이 마모품이고 한계(p[3])에 닿았나 / 10 % 안으로 다가왔나
  const top = parts[0];
  ...
  const stLabel = LABEL[u.st] === '정상' ? '정상 가동' : LABEL[u.st];
  ...
  return (
    ...
    <div class="stat">
      ...
      <div class="st ${u.st}"><i></i><b>${stLabel}</b>${stWhy}${stWhen}</div>
    </div>
    ${driverRow}
    ${aiBanner}
    <div class="body">
      ...
      <details class="fold"><summary><span>서류</span><span class="hint">차량 ${u.docs.length}건 · 운전자 4건</span><span class="chev"></span></summary>
        <div style="padding:0 18px 14px">
          ...
          <div class="docs">${[
            ...
            .map(d => `<a href="#" onclick="return false">${d[0]} <span>PDF · ${d[1]}</span></a>`)
            .join('')}</div>`
              : ''
          }
        </div></details>
...
function placeBadges() {
  ...
  for (const p of pts) {
    const el = p.s.marker.getElement()?.querySelector('.mk');
    if (!el) continue;
    const dx = p.x - p.ox,
      dy = p.y - p.oy,
...
function buildSiteLayer() {
  ...
  for (const u of s.units) {
    ...
    const m = L.marker(ll(s, u.d), { icon, title: u.num + '호기', zIndexOffset: u.st === 'run' ? 0 : 500 }).addTo(
      siteLayer,
    );
...
document.addEventListener('mouseover', e => {
  ...
  if (!t) return;
  if (t.dataset.site) sync('site', t.dataset.site, true);
  if (t.dataset.unit) sync('unit', t.dataset.unit, true);
});
document.addEventListener('mouseout', e => {
  const t = (e.target as any).closest('[data-site],[data-unit]');
  if (!t) return;
  if (t.dataset.site) sync('site', t.dataset.site, false);
  if (t.dataset.unit) sync('unit', t.dataset.unit, false);
});
