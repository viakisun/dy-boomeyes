<script lang="ts">
  // A1-05 장비 상세 (specs/video-basics AC-3) — 텔레메트리 · 소모품 · 서류 완비율/만료 · 저장 영상(프로파일 소스 탭) · 카메라
...
<div class="gap-stack-md flex flex-col" data-scr={SCR['A1-05']}>
  ...
  <section class="rounded-card border-border bg-surface p-inset-md gap-stack-sm flex flex-col border" aria-label="서류">
    <div class="flex items-center justify-between">
  ...
  <section class="gap-stack-sm flex flex-col" aria-label="저장 영상">
    ...
    <Tabs
      ...
      value={data.source}
      onchange={(id) =>
        goto(resolve(`/a1/monitor/${d.id}?source=${id}` as '/'), {
          keepFocus: true,
          noScroll: true,
          replaceState: true,
        })}
