...
| 3 | `WebShell` · `PwaShell` | AC-3 · AC-4 · AC-6(W1: 루트 `data-theme` · `?theme=` 쿼리 · 웹 탑바 토글(localStorage) · PWA 시스템 추종 · `capture --dark` 14장 · e2e `web-theme`/`pwa-theme` 6) · `maintenance` 로그인 경로 — 완료(B0-01 roles + `login-maint` 카드 → `/b1/escalation`, e2e) | FR-024 |
...
| 8 | 브랜드 마크(W2.6 L) — `Logo` mark/glyph/lockup · 사이드바 접힘 글리프 · 로그인 두 톤 · favicon·PWA 아이콘 `brand:build` · DY-design §13 | AC-8 — e2e `web-brand`·`pwa-brand`·`pwa-install`(favicon·아이콘 200) · 기준선 41장 재등록 | DISC-021 |
