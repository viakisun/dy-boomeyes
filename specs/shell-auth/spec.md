...
## 수용 기준
- **AC-1** Given 데모 계정 7(`roles.yaml demo_account`) When 각 계정으로 로그인 Then 역할별 첫 화면으로 이동한다 — control→`/b1/dash`, hq-safety→`/b2/map`, site-safety(웹)→`/b3/console`, ops-admin→`/b4/protocols`, maintenance→`/b1/escalation`(B1 연계 카드), driver→`/a2/today`, site-safety(앱)→`/a1/inbox`, hq-safety(앱)→`/a3/sites`, owner→`/a4/fleet` [FR-001]
...
- **AC-9** Given 웹 로그인 카드 When 소유주 카드(`owner01`, 첫 카드)를 누르면 Then B1-02로 들어가고 세션에 `ownerId`(OWN-001)가 실리며 탑바 org는 '사업주 소속', 제조사 3역할(control·ops-admin·maintenance)의 org는 '제조사 (DY)'다 — 픽스처 `login-owner`(ADR-012) [FR-001, FR-024]
## 상태 픽스처
