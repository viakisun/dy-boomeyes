...
| # | 작업 | DoD | Refs |
|---|---|---|---|
| 1 | ui `CheckinCard` · `ChecklistForm` · `TelemetryGauge` · `TelemetryStrip` (임계 색은 `sys.color.status.*`) | 데모 렌더 · `tokens:lint` 0건 · 다크 렌더 | ADR-004 |
...
순서 1 → 2 → 3 → 4 → 5 → 6. 완료(W2 B7): A2-06 메뉴·현장 정보(today 기반 · 동의 요약 · 앱 정보, 편집 없음). AC-6 오프라인 아웃박스(`A2-02:queued`) 완료(W2 B12 · ADR-010): `packages/offline` · `?net=` · 셸 배너·pill · e2e 4 · Vitest 7.

커밋 예: `feat(driver): A2-02 오늘 · 출근 체크인 — Refs: SCR-A2-02 FR-013`.
