...
- 공통 패턴은 FleetSummary(하나의 전체+분포), EquipmentRow(식별자·현장 2줄+상태·행동), ContextHeader, DocumentViewer. cmp/sys 토큰만 소비한다. 카드 9개를 나열하지 않는다. 넓은 화면은 비교 가능한 열, 좁은 화면은 모든 중요 정보가 남는 줄바꿈이다.
