...
## 레이아웃
- **A1-02 업무함**(PwaShell): 상단 요약 칩(new · in-progress · escalated 건수) · 필터 바(`Tabs` 상태 + `Menu` 유형, 활성 칩) · 업무 `List` 행 = 유형 아이콘 · 제목 · `StatusPill`(task 상태) · 기한 D-n · 심각도 `StatusDot`. 알림 배지는 AppBar 액션.
...
- **B1-04 에스컬레이션**: 상단 `Banner`(규칙: 미접수 1h → 본사 안전관리자 통보) · `DataTable`(업무 · 현장 · 경과 · 통보 대상 · 통보 시각).

## 컴포넌트 · 토큰
신규 ui: `DataTable`(행 밀도 36/48 · 정렬 · 포커스 행 · Enter 선택) · `List`/`ListRow` · `Timeline` · `FilterBar`(Tabs + Menu 조합). 기존: `StatusPill` `StatusDot` `Badge` `Banner` `EmptyState` `Toast` `Dialog`. 토큰: `cmp.table.*` `cmp.card.*` · severity = `sys.color.status.*`(SEVERITY_TONE) · task 상태 = `domain.task.*`(TASK_TONE).
...
## 열린 질문
DISC-035(조치 보고 입력 채널) · DISC-036(알림 수신자·방식) — 통보 대상 기본값은 본사 안전관리자.
