...
- **A1-05 장비 상세**: 헤더(호기 · 상태 · 현장) · `TelemetryStrip` · 소모품 `Gauge` 2 · 서류 완비율 카드(만료 임박 D-27 `Badge`) · 저장 영상 `Tabs`(프로파일별 소스) + 목록(날짜 · 길이 · 소스 · 재생 자리) · 카메라 타일 2.
...
## 컴포넌트 · 토큰
video 패키지: `VideoPlayer`(muted · autoplay · playsinline · 정지 버튼 · 소스 전환) · `SnapshotPoller`(5~10s) · `BboxOverlay`(SVG, 정규화 좌표) · `HealthBadge`(camera 상태기계 라벨 `labels.ts`). ui: `Gauge` `TelemetryStrip` `Tabs` `Badge` `EmptyState`. 토큰: `sys.color.media.*`(테마 무관) · `domain.video.*` · `domain.equipment.*`.
...
- `packages/domain/profile.ts`: `profileFlags(profile)` → { sources: ['server'|'sd'|'nvr'], snapshotEveryMs, sdRecall, nvrTimeline }. Vitest `[FR-005]`(QA 게이트 5 "프로파일→피처플래그"). camera 상태기계 테스트 `[FR-034]`.
