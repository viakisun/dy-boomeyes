...
| # | 작업 | DoD | Refs |
|---|---|---|---|
| 1 | `packages/domain/protocol.ts` — `validateProtocol` · `parseSample` (cpb.v0.1 필드 · 샘플 4종) | Vitest `[FR-020]` 정상/누락/타입 오류 | FR-020 |
| 2 | mock: 프로토콜 2(운영 · 테스트) · 샘플 4 · 규칙 8 · 고장코드 · 이력 · 픽스처 `proto` `rules` | `?state=` 재현 | ENT-11 ENT-08 |
| 3 | ui `ProtocolUploader`(`FileUpload`) · `RuleThresholdRow` | 데모 렌더 · `tokens:lint` 0건 | ADR-004 |
